...
// productivity calculation needs the SAME test, and that calculation runs on the
// backend too (the Final-EOD freeze). Keeping the implementation here meant
// productivity could not reach it, which is why a "Closed - Canceled" call was
// scored as a completed close in Engineer Productivity while every other surface
// excluded it. Re-exported so existing imports of this module keep working.
export {
  classifyFlexClosureOutcome,
  hasFlexClosureOutcome,
  isCancelledClosure,
  type FlexClosureOutcome,
